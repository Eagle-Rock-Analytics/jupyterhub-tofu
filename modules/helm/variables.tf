...
variable "lifecycle_post_start_command" {
  description = "Command to run after container starts: 1) Creates persistent user kernel with PYTHONPATH for editable installs, 2) Installs latest climakitaegui GUI, 3) Clones notebooks. The kernel uses PYTHONPATH to ensure user site-packages and editable installs take precedence over system packages."
  type        = list(string)
  default = [
    "sh",
    "-c",
